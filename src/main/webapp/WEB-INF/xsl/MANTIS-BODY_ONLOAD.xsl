...
<!--MANTIS-BODY_ONLOAD.xsl-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">

<xsl:output method="html"/>
<xsl:template name="MANTIS-BODY_ONLOAD">

<!--REQUIRED by MANTIS -->
<xsl:attribute name="onLoad">
<!-- Sets the cursor and focus of the Screen -->
<!-- Comment out to disable autofocus -->
<xsl:choose><xsl:when test="//@CUR">
setCursor('<xsl:value-of select="//@CUR"/>');
</xsl:when><xsl:otherwise>
<xsl:value-of select='normalize-space("setCursor();")'/>
</xsl:otherwise></xsl:choose>

<xsl:choose><xsl:when test="//CONTROL-TIMEOUT">
  <xsl:text>setTimeout('timeout()', '</xsl:text><xsl:value-of select="//CONTROL-TIMEOUT"/><xsl:text>');</xsl:text>
</xsl:when></xsl:choose>
<xsl:choose><xsl:when test="//@PROMPT">
...
if(typeof M$listMore == 'function') M$listMore( '<xsl:value-of select="//@NAME"/>' );
</xsl:attribute>
